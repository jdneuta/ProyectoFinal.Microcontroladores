...
void I2C_Init_Master(unsigned char sp_i2c)
{
    ...
    SSPSTAT = sp_i2c;               //Variable para frecuencia    
...
void I2C_Start(void)
{
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
    SSPCON2bits.SEN = 1;            //Genera condicion de start
    while(PIR1bits.SSPIF == 0);     //Espera a que termine start
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
}               
...
void I2C_Stop(void)
{
    ...
    while(PIR1bits.SSPIF == 0);     //Espera a que termine stop
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
}
...
{
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
    SSPCON2bits.RSEN = 1;           //Genero condicion de reseteo 
    while(PIR1bits.SSPIF == 0);     //Espera a que termine reseteo
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
}
...
{
    PIR1bits.SSPIF = 0;             //Limpia la bandera de interrupción del módulo MSSP
    SSPCON2bits.ACKDT = 0;          //Elige ACK porque es 0
    SSPCON2bits.ACKEN = 1;          //Inicializa la secuencia en SDA /SCL 
...
{
    PIR1bits.SSPIF = 0;              //Limpia la bandera de interrupción del módulo MSSP
    SSPCON2bits.ACKDT = 1;           //Habilita NACK porque es 1 
    SSPCON2bits.ACKEN = 1;           //Inicializa la secuencia en SDA /SCL   
    while(PIR1bits.SSPIF == 0);
    PIR1bits.SSPIF = 0;              //Limpia la bandera de interrupción del módulo MSSP
}
...
{
    PIR1bits.SSPIF = 0;              //Limpia la bandera de interrupción del módulo MSSP
    SSPBUF = data;                   //Almacena datos mientras se envia 
    while(PIR1bits.SSPIF == 0);
    PIR1bits.SSPIF = 0;              //Limpia la bandera de interrupción del módulo MSSP
    return SSPCON2bits.ACKSTAT;      //Indica si recibe o no  algo del esclavo
}
...
unsigned char I2C_Read(void)
{
    ...
    while(PIR1bits.SSPIF == 0);      //Espera que termine de recibir
    PIR1bits.SSPIF = 0;              //Limpia la bandera de interrupción del módulo MSSP
    return SSPBUF;                   //Retorna lo que recibe 
}
#endif
